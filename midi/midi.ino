...
const uint16_t triggerThresholds[numPads] = {650, 650, 650, 650, 650, 650, 650, 650, 650, 650}; // Threshold iniciais {pad1, pad2, pad3, pad4}
        ...
        if (hitOccurredRecently) {
          ...
          if (learning) {
            ...
            if (msPassed >= tailLength) {
              ...
              uint8_t midiVelocity = min(127, ((highestYet >> midiVelocityScaleDownAmount) + 1));
              uint16_t nota = notes[padNo];
              usbMIDI.sendNoteOff(nota, midiVelocity, 1);
            }
            else {
              ...
              if (previousHitStillRecent) {
                ...
                if (msPrevious >= tailLength) { 
                  previousHitStillRecent = false;
                } // Se o gap entre o ultimo hit for maior que 128ms, desliga a flag -> "previousHitStillRecent"
                else {
...
void setup() {
  for (int i = 0; i < numPads; i++) { // loop iniciará a função init()"iniciar" para todos os pads
    pads[i].init(i, 0);
  }
